# 本着不改变原来代码结构的情况 重新设计适配于具有参数共享和qk分布loss的代码
import numpy as np
import torch
import torch.nn as nn
import torch.nn.functional as F
from Model.CVAE_utils import Normal, MLP2, MLP
from Model.star import TransformerModel, TransformerEncoder, TransformerEncoderLayer, _get_clones
from Model.star_cvae import Decoder, STAR_CVAE
from Model.DualTT import PositionalAgentEncoding


# 基于TransformerEncoder 重写对应的 TransformerEncoder 并输出对应的attn, qkv_dict
...
class Dual_TTAligin_Encoder(nn.Module):
    ...
    def __init__(self, args, stage, dropout_prob=0):
        ...
        self.temporal_encoder_1 = TemporalTransformerEncoder(TransformerEncoderLayer(d_model=32, nhead=8), 1)
        self.temporal_encoder_2 = TemporalTransformerEncoder(TransformerEncoderLayer(d_model=32, nhead=8), 1)
    ...
    def forward(self, inputs):
        ...
        temporal_input_embedded_temporal_TS,att_TS,qkv_TS = self.temporal_encoder_1(temporal_input_embedded_origin,need_aligin_loss=self.args.need_aligin_loss)
        ...
        # TM branch  添加对应的att,qkv
        temporal_input_embedded_temporal_ST ,att_ST, qkv_ST= self.temporal_encoder_2(spatial_input_embedded_spatial_ST,need_aligin_loss=self.args.need_aligin_loss)
        ...
        return fusion_feat,loss,qkv_dict

class Dual_TTAligin(STAR_CVAE):
    def __init__(self, args):
        super(Dual_TTAligin, self).__init__(args)
        self.args = args
        self.embedding_size = 32
        # model structure
        self.past_encoder = Dual_TTAligin_Encoder(args, stage='past')
        self.future_encoder = Dual_TTAligin_Encoder(args, stage='future')
    ...
    def forward(self, inputs, stage):
        ...
        past_feature,past_loss,past_qkv_dict = self.past_encoder(past_traj)
        future_feature,future_loss,future_qkv_dict = self.future_encoder(future_traj)
    ...
    def inference(self, inputs):
        ...
        past_traj = nodes_current[:obs_length], nodes_abs_position[:obs_length], nei_list[:obs_length]
        # encoder
        past_feature,_,_ = self.past_encoder(past_traj)
