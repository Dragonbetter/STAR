...
class DatasetProcessor_ETH_Apollo(DatasetProcessor_BASE):
    ...
    def data_preprocess_for_ApolloScale(self, data_path, window_size, stride):
        ...
        data['rec&trackId'] = [recId + '_' + str(trackId).zfill(4) for recId, trackId in
                                   zip(data.sceneId, data.trackId)]
        ...
        print('切分断开的轨迹')
        data_continues = split_fragmented(data)
        ...
        # drop track_id = 》一个可能的选择是后续这个需要保留，因为需要基于这个去返回查询对饮给的场景和id
